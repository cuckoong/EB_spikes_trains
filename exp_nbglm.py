...
def my_mse_fun(clf, X, y):
    x0 = clf['clf'].op.x
    X_transform = clf['scale'].transform(X)
    y_pred = clf['clf'].predict(X_transform)
...
if __name__ == '__main__':
    ...
    files = ['62423.mat']#, '62871.mat']#'62os.listdir(wkdir)
    for file in files:
        ...
        for i in range(spike_count.shape[1]):
            ...
            pipe_op = Pipeline([
                ('scale', StandardScaler()),
                ('clf', NBGLM(lam=grid.best_params_['clf__lam']))])
            # cv
        ...
        df = pd.DataFrame(df)
        df.to_csv('file_{}_neuron_nbglm_true.csv'.format(file[:-4]))

        df2 = dict(lam=lam_list)
        df2 = pd.DataFrame(df2)
        df2.to_csv('file_{}_neuron_nbglm_lam.csv'.format(file[:-4]))
