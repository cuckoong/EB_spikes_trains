...
class NBGLM0(BaseEstimator):
    ...
    def simulate(self, n_sim, n_bin, n_neuron, seed_beta=42, seed_X=42, r=None, density=0.2, get_y_true=True):
        # fixed link function
        """
        :param n_sim: simulation trial
        :param n_bin: data length
        ...
        np.random.seed(seed_X)
        X = np.random.normal(0.0, 1.0, [n_bin, n_neuron])
        mu = 1/(np.exp(beta0 + np.dot(X, beta)) + 1)
        y_true = r * (1 / mu - 1)  # true y value
    ...
    def der(self, x0, X, y):
        ...
        mu = 1/(g_est + 1)
        mu_gam = g_est + 1
        der_mu_over_gam = (np.log(mu_gam) - g_est/ mu_gam) * mu

        der_mu_over_beta = -1 * X * (g_est * mu / mu_gam).reshape(-1, 1)
        der_mu_over_beta0 = -1 * g_est * (g_est + 1) ** (-2)

        A = r / mu - y / (1 - mu)
        der[0] = -1 * np.sum(digamma(r + y) - digamma(r) + np.log(mu + np.finfo(np.float32).eps))
        der[1] = -1 * np.sum(A * der_mu_over_beta0)
        der[2:] = -1 * np.sum(der_mu_over_beta * A.reshape(-1, 1), axis=0) + \
                  self.lam * (self.alpha * np.sign(beta) + (1 - self.alpha) * beta)
        return der
    ...
    def predict(self, X):
        ...
        beta0_op = op['x'][1]
        beta_op = op['x'][2:]
        mu_op = 1/(np.exp(beta0_op + np.dot(X, beta_op)) + 1)
        y_op = r_op * (1 / mu_op - 1)  # estimated y value
        return y_op
